...
def register_user():
    ...
    try:
        ...
        conn.close(); return jsonify({"error": "Usuario ya existe"}), 409
    except Exception as e:
        conn.close(); print(f"Error registro: {e}"); return jsonify({"error": "Error interno"}), 500
...
def login_user():
    ...
    try:
        ...
        if user and bcrypt.check_password_hash(user['password_hash'], password):
            # --- ¡LA CORRECCIÓN! ---
            # Convertimos el user['id'] (que es INTEGER) a string
            user_id_string = str(user['id'])
            access_token = create_access_token(identity=user_id_string) # Pasamos el string
            # --- FIN CORRECCIÓN ---
            return jsonify({"mensaje": f"Login exitoso", "access_token": access_token}), 200
        else:
            return jsonify({"error": "Credenciales incorrectas"}), 401
    except Exception as e:
        print(f"Error login: {e}"); return jsonify({"error": "Error interno"}), 500

# --- (Ruta de Preguntas - sin cambios) ---
...
def get_preguntas_por_tema(tema):
    ...
    try:
        ...
        if not rows: return jsonify({"error": f"No hay preguntas para '{tema}'"}), 404
        for row in rows:
        ...
        print(f"Error preguntas: {e}"); return jsonify({"error": "Error al cargar"}), 500

# --- (Ruta de Perfil - sin cambios) ---
@app.route("/api/user/profile", methods=['GET'])
@jwt_required()
def get_user_profile():
    user_id_string = get_jwt_identity() # Ahora get_jwt_identity() devuelve un string
    try:
        # Podemos convertirlo de vuelta a int si lo necesitamos para la DB
        ...
        print(f"Error perfil: {e}"); return jsonify({"error": "Error interno"}), 500

# --- (Ruta de Guardar XP - sin cambios) ---
...
def update_user_xp():
    ...
    try:
        ...
        print(f"Error update XP: {e}"); return jsonify({"error": "Error interno"}), 500
